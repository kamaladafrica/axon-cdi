...
/**
 * Copy of AggregateConfigurer
 * Just changed the used of instanceof on 
 * @author damien
 *
 * @param <A>
 */
public class AggregateConfigurerUsingProxies<A> implements  AggregateConfiguration<A> {
     ...
     */
    public static <A> AggregateConfigurerUsingProxies<A> defaultConfiguration(Class<A> aggregateType) {
        return new AggregateConfigurerUsingProxies<>(aggregateType);
    }
     ...
     * @return An AggregateConfigurer instance for further configuration of the Aggregate
     */
    public static <A> AggregateConfigurerUsingProxies<A> jpaMappedConfiguration(Class<A> aggregateType,
                                                                    EntityManagerProvider entityManagerProvider) {
        return new AggregateConfigurerUsingProxies<>(aggregateType).configureRepository(
     ...
     */
    protected AggregateConfigurerUsingProxies(Class<A> aggregate) {
        ...
        repository = new Component<>(() -> parent, "Repository<" + aggregate.getSimpleName() + ">", c -> {
        	Assert.state(c.eventBus() instanceof EventStore
        			|| c.eventBus().toString().equals(EventStore.class.getName()),
            ...
            if (c.commandBus().toString().equals(CommandBusProxified.class.getName())) {
            	CommandBusProxified commandBusProxified = (CommandBusProxified) c.commandBus();
            	if (commandBusProxified.isDisruptorCommandBus()) {
            		return commandBusProxified.createRepository(aggregateFactory.get(), c.parameterResolverFactory());
            	}
            }
     ...
     */
    public AggregateConfigurerUsingProxies<A> configureRepository(Function<Configuration, Repository<A>> repositoryBuilder) {
     ...
     * @return this configurer instance for chaining
     */
    public AggregateConfigurerUsingProxies<A> configureAggregateFactory(
     ...
     * @return this configurer instance for chaining
     */
    public AggregateConfigurerUsingProxies<A> configureCommandHandler(
     ...
     * @return this configurer instance for chaining
     */
    public AggregateConfigurerUsingProxies<A> configureCommandTargetResolver(
